...
void init(void)
{
    ...
    {
        for(int column = 0, counter = ((d - row) * (d)) - 1; column < d; column++)
        {
    ...
    //adjust if there are an odd number of tiles
    if((d * d) % 2 == 0)
    {
        board[d - 1][d - 3] = 1;
        board[d - 1][d - 2] = 2;
...
bool won(void)
{
    //variable used to check grid beginning at [0][0]
    int counter = 1;
    ...
    for (int row = 0; row < d; row++)
    {
        for (int column = 0; column < d; column++)
        {   
            //change counter to 0 at bottom right corner, otherwise grid will never return true
            if (board[row][column] == 0)
            {
                counter = 0;
            }
            
            //check the rest of the numbers for correctness           
